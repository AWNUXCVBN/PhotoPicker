...
public class ImageLoader {
	...
	private void initDiskCache() {
		try {
			File cacheDir = OtherUtils.getDiskCacheDir(MainActivity.context, "images");
			if (!cacheDir.exists()) {
				cacheDir.mkdirs();
			}
			mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(MainActivity.context), 1,
					15 * 1024 * 1024);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/*
	 * 此处的源代码是，用的时候滑动会崩溃，所以我这么改了一下，本人不怎么懂java private boolean
	 * cacheBitmap2Disk(Bitmap bitmap, OutputStream outputStream) {
